...
keep_aspect_ratio = st.sidebar.checkbox("Сохранять пропорции 1:1", value=True)
...
if formula_input:
    try:
        ...
        hover_labels = [
            f"X: {x:.1f}<br>Y: {y:.2f}" if not np.isnan(y) else "Разрыв"
        ...
        fig.update_layout(
            title=f"График функции: y = {formula_input}",
            xaxis=dict(
                range=[x_min, x_max],
                zeroline=False,
                showgrid=True,
        ...
        st.error(f"Не удалось построить график. Проверьте правильность введенной формулы.\nДетали ошибки: {e}")
